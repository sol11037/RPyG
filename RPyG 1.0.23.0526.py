...
#class damageexp : #피해량 수식 클래스
    #def __init__(self,first,second) : #1st에 공격자 공격력, 2nd에 피격자 방어력이 들어감
        #self.first = first
        #self.second = second
    #def attack(self) :
        #result = int(self.first - self.second/2)
        #if result < 1 : result = 1
        #return result
        #피해량 계산식을 클래스로 구현하려다 처참히 실패한 흔적
...
while 0 < slimehp and yourhp :
    ...
    while True : #행동 선택문
     ...
     if choice == '1' : #평타
         print("'당신'의 공격!")
         damage = int(youratk-slimedef/2) # 피해량 기본 계산식
         if damage<0 : damage = 1 #최소 피해량은 1
         ...
         while True : #스킬 선택문
             ...
             if skill == '0' : #sp 없을 때 탈출용
                 ...
                 if yoursp < 3 : print("sp가 모자라다!")
                 elif gitti == True : print("'당신'은 이미 이를 악물고 있다!")
                 else :
                     gitti = True
                     gittilast = 3
                     print("'당신'은 이를 악물었다!")
                     ...
                     if yoursp < 10 : print("sp가 모자라다!")
                     elif yourhp == yourhpmax : print("'당신'의 hp는 이미 최대다!")
                     else :
                         print("'당신'의 응급처치!")
                 ...
                 if yoursp<15 : print("sp가 모자라다!")
                     ...
                     yoursp = yoursp - 15
                     damage = int(youratk*1.2 - slimedef/2)
                 ...
                 if yoursp < 30 : print("sp가 모자라다!")
                     ...
                     yoursp = yoursp - 30
                     damage = int(youratk*2 - slimedef*0.2)
                     if damage < 1 : damage = 1
                     slimehp = slimehp - damage
                     print("'당신'의 '개쎄게 베기'!")
                     print("'허접해 보이는 슬라임'에게",damage,"피해!")
    ...
    if slimehp <= 0 : print("'허접해 보이는 슬라임'은 쓰러졌다!")
        ...
        if slimeacidcooltime >= 4 : #slimeacid
            ...
            damage = int((slimeatk*0.7)-yourdef/2)
            if defend == True : damage = int(damage/2)
            if damage < 1 : damage = 1
            print("'당신'에게",damage,"피해!")
            yourhp = int(yourhp - damage)
            slimeacid = 3
            yourdef = yourdef-5
